<script lang="ts">
	...
	// Props: audio source URL, subtitle content, and format ("lrc", "srt", "vtt", "smi", or "plain")
...
function parseSrt(content: string) {
  ...
  function parseVttTime(timeStr: string) {
	...
	function updateHighlight() {
		...
		currentTime = (audioPlayer.seek() as unknown as number) || 0;
	...
	function togglePlayback() {
		if (!audioPlaying) {
			audioPlayer.play();
			audioPlaying = true;
      // 如果subtitles最后一行的end是Infinity
      if (subtitles[subtitles.length - 1].end === Infinity) {
        // 将subtitles最后一行的end设置为audioPlayer.duration(0)值
		subtitles[subtitles.length - 1].end = audioPlayer.duration(0) || Infinity;
      }
			updateHighlight();
		} else {
			audioPlayer.stop();
	...
	onMount(() => {
		...
		// Create Howl instance with looping enabled.
